...
export const AddComment = ({ aboutPostId, onCommentAdded }) => {
    ...
    const onAddComment = async ev => {
        ev.preventDefault()
        try {
            await commentService.add({ txt, aboutPostId })
            setTxt('')
            onCommentAdded()
        } catch (err) {
    ...
    return <form className="add-comment flex a-center">
        <button>
        ...
        <textarea aria-label="Add a comment…" placeholder="Add a comment…" autoComplete="off" autoCorrect="off" value={txt}
            onKeyDown={ev => ev.key === 'Enter' && onAddComment(ev)} onChange={ev => setTxt(ev.target.value)}></textarea>
        <button className={txt || 'disabled'} onClick={onAddComment}>Post</button>
    </form>
}