...
const _PostPreview = ({ post, loggedInUser }) => {
    ...
    const onAddComment = async ev => {
        ev.preventDefault()
        try {
            await commentService.add({ txt: newCommentTxt, aboutPostId: post._id })
            setNewCommentTxt('')
            setComments(await commentService.query(post))
        } catch (err) {
            // SNACKBAR DOCKED TO BOTTOM
    ...
    return <article className="post-preview">
        ...
        <section className="social-container">
            <header className="flex j-between a-center">
                <div>
                    <button onClick={onToggleLike}>
                        {myLike ? <svg aria-label="Unlike" className="_8-yf5 " fill="#ed4956" height="24" viewBox="0 0 48 48" width="24">
                ...
                <button>
                    <svg aria-label="Save" className="_8-yf5 " fill="#262626" height="24" viewBox="0 0 48 48" width="24">
            ...
            {likes.length ? <label className="lbl-likes">{getLikeSyntax()}</label> : null}
            <div className="comment-section">
                ...
                <div>{comments.length > 2 &&
                    <Link to="/TEMP" className="muted fw400">View all <span>{comments.length}</span> comments</Link>}</div>
            ...
            <form className="add-comment-container flex a-center">
                <button>
                    <svg aria-label="Emoji" className="_8-yf5 " fill="#262626" height="24" viewBox="0 0 48 48" width="24">
